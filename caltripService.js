...
function readEvents(req, res, next) {
    db.many("SELECT id, title, description, startDate::TIMESTAMP::DATE, endDate::TIMESTAMP::DATE, location, price, category FROM TheEvent ORDER BY startDate ASC")
        .then(data => {
            res.send(data);
        })
        .catch(err => {
...
function readEvent(req, res, next) {
    db.oneOrNone("SELECT id, title, description, startDate::TIMESTAMP::DATE, endDate::TIMESTAMP::DATE, location, price, category FROM TheEvent WHERE id=${id}", req.params)
        .then(data => {
